...
{
  FILE *file;
  char* fn = alloc_printf("%s/log_data", out_dir);
  file = fopen(fn, "a");
  fprintf(file, "%d", *num);
  fprintf(file, "\n");
  fclose(file);
}
void pw(char* mem, const char* ffn)
{
  FILE *file;
  char* fn = alloc_printf("%s/%s", out_dir, ffn);
  file = fopen(fn, "a");
  fprintf(file, "%s", mem);
  fprintf(file, "\n");
  fclose(file);
...
static u8* cal_s(u64 cur_ms, u64 event_ms);
    ...
    if (state_count > 1) {
      ...
      for(i=1; i < state_count; i++) {
        ...
        Agnode_t *from, *to;
          ...
          // char* trans = alloc_printf("%d   %ds", state_ids_count, cal_s(cur_ms, start_time));
          // pw(trans);
        }

        u64 cur_ms = get_cur_time();
        char* trans = alloc_printf("%d to %d   %ds",prevStateID, curStateID, cal_s(cur_ms, start_time));
        pw(trans, "state_trans");
...
#define SP20    SP10 SP10
  ...
  if (last_time != cal_s(cur_ms, start_time))
  {
    last_time = cal_s(cur_ms, start_time);
    char* path_stat = alloc_printf("%d  %d", queued_paths, last_time);
    pw(path_stat, "path");
  }
